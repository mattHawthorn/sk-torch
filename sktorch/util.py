...
def efficient_batch_iterator(X: Iterable[T1], y: Opt[Iterable[T2]]=None,
                             ...
                             num_workers: int=0) -> Iterable[Tuple[Tensor, Tensor]]:
    if y is None:
        y = X
...
class SeqDataset(Dataset):
    ...
    def __init__(self, X_seq: Seq[T1], y_seq: Seq[T2],
                 X_encoder: Opt[Callable[[T1], Tensor]]=None, y_encoder: Opt[Callable[[T2], Tensor]]=None):
...
class BatchedIterableDataloader:
    ...
    def __iter__(self) -> Iterable[Tensor]:
        ...
        return batched(zip(X_, y_), batch_size=self.batch_size)


class MappedDataLoader(DataLoader):
    def __init__(self, dataset: Dataset, X_encoder: Opt[Callable[[T1], Tensor]]=None, y_encoder: Opt[Callable[[T2], Tensor]]=None,
                 **kwargs):
